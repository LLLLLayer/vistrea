...
The native SwiftUI Snapshot workspace is a standalone Swift Package. It currently provides:
...
- a persisted Runtime event timeline pane with newest-first ordering, transient payload summaries, and the Host event-pump state from `GET /v1/status` in the pane header (its epoch, persisted sequence, and error code in the tooltip);
...
- Canvas Screen State details: clicking a state opens a side panel with its persisted title, kind, status, first/last seen, and canonical Snapshot ID, shows how many observations the state deduplicates, lists the Wiki nodes already linked to it, and can create a `relates_to` Wiki link to the state;
