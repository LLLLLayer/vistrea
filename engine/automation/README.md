...
`WdaAutomationProvider` is the real iOS adapter: it wraps a loopback WebDriverAgent endpoint (the XCUITest-hosted driver Appium uses) over the W3C actions protocol with logical-point coordinates, types through `/wda/keys`, launches through `/wda/apps/launch`, performs `back` as the interactive left-edge pop gesture because iOS has no system back key, lazily creates and recreates the driver session exactly once on invalidation, re-verifies the bound authorization digest and expiry before touching the wire, and reports injected input as `uncertain`. The wire protocol is verified against a local fixture driver; the opt-in `pnpm test:e2e:ios-real-automation` acceptance boots an operator-provided WebDriverAgent checkout (`VISTREA_WDA_PROJECT`) against a dedicated Simulator and awaits real-device verification.
