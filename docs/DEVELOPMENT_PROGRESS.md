...
- The raised iOS automation acceptance (storefront walk) has not yet had a green device run: the host machine was saturated by an unrelated real-device session during every attempt. Rerun `VISTREA_WDA_PROJECT=... pnpm test:e2e:ios-real-automation` on a calm machine; the Android twin of the same raise is green.
