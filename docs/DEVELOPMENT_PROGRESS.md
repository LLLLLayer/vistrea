...
| iOS Runtime SDK and Demo App | `uikit-runtime-loop-1` | In progress | Canonical models, all shared scenarios, UIKit hierarchy/PNG capture, Debug Inspector, hardened Runtime connection, verified first Snapshot loop, negotiated event streaming, and protected alpha tuning previews exist; automatic UIKit event observation and the SwiftUI adapter remain | 22 Swift tests in Debug and Release, 6 Node/Swift interop tests, real iOS E2E; commits `4c67fb2`, `be60c0b`, `7671364`, `74967a6`, `7824f29`, and `3511bb5` |
...
| Vistrea Studio | `snapshot-studio-1` | In progress | Native SwiftUI Host status, capture/list, screenshot, 2D tree, node details, scenario/build/source context, Runtime event timeline, Review Issues, Screen State Canvas, Deep Wiki search, SceneKit 3D Layer Inspector, fixture mode, and production acceptance probe; editing workflows and exploration control remain | 28 Studio tests and Release build; commits `6ccf9f2`, `534517d`, `fa9bf88`, `c62694e`, and `85233cb` |
| Agent integrations | `agent-surfaces-2` | In progress | One authenticated Host client exposes 45 operations spanning capture, events, design review, tuning, Screen Graph, Deep Wiki, validation, build diffs, and portable packs through the strict JSON CLI, 44 stdio MCP tools, four agent skills, and the headless CI gate with severity thresholds and machine-readable reports; exploration sessions and readable exports remain | Agent adapter round trips and 3 CI gate integrations; commits `2e1d157`, `e9d1f77`, and `914a450` |
