...
| iOS Runtime SDK and Demo App | `uikit-runtime-loop-3` | In progress | Canonical models, all shared scenarios, UIKit hierarchy/PNG capture, Debug Inspector, hardened Runtime connection, verified first Snapshot loop, negotiated event streaming, protected reversible alpha/color/font/spacing/insets/corner-radius tuning previews, and SwiftUI content captured as per-element accessibility nodes (requires an active accessibility runtime) exist; automatic UIKit event observation remains | 33 Swift tests, broader Runtime controller build coverage, 13 UIKit-gated tests on a booted Simulator, 6 Node/Swift interop tests, and real iOS E2E; physical-device acceptance of every broader property remains |
| Android Runtime SDK and Demo App | `android-view-runtime-loop-3` | In progress | Canonical models, all shared scenarios, View/ViewGroup capture, Debug Inspector, protected Runtime connection, Release exclusion, verified first Snapshot loop, negotiated event streaming, protected reversible alpha/color/font/spacing/insets/corner-radius tuning previews, and full Compose semantics-tree capture through a registered view-semantics extension exist; automatic View event observation remains | Connection tests, Debug/Release builds, Android Lint, release-boundary verification, Node/Kotlin interop, and real Android E2E; physical-device acceptance of every broader property remains |
| Vistrea Studio | `snapshot-studio-6` | In progress | Native SwiftUI Canvas-first workspace with exact Application Version + Build scope, build-local canonical Snapshot inspection, screenshot, 2D tree, hierarchy-depth 3D, design comparison, broader Debug tuning editors, annotations, Wiki linking and editing, Screen State-scoped Review Issues, event timeline, live exploration progress, Canvas merge/split curation, fixture write parity, and a production acceptance probe; direct difference promotion and recapture verification are available through the Engine/Host/CLI but are not yet dedicated Studio controls | 105 Studio tests, scoped Host/Engine/Data integrations, and Release build |
...
- Difference-to-Issue promotion, source suggestions, visual baselines, and automatic recapture/re-verification are implemented through Engine, Host API, and CLI. Dedicated Studio controls for direct Difference promotion and one-click recapture verification, plus Studio Collection management, remain follow-up UI work.
...
- The CLI's `driver ios up --device <udid>` signs and boots WebDriverAgent on a real device (team from flag, app project, or Keychain) but is unverified on hardware, and real-device exploration additionally needs the Runtime SDK to reach the Host from the device: the Runtime transport is loopback-only by design, so a TLS non-loopback mode (mirroring the Hub's) is the missing piece of the physical-device vertical.
...
1. Exercise the broader tuning property matrix on real iOS and Android devices and add dedicated Studio controls for Difference promotion and recapture verification.
2. Form the physical-iPhone Runtime-to-Host vertical and strengthen the iOS Release artifact boundary.
