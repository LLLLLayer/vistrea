...
| Machine-readable protocol | `v1-pre-release-2` | Verified | Complete shared `DataUnitOfWork` value surface, strict JSON, semantic graph rules, and compatibility fixtures | 84 fixtures and 24 protocol contract tests |
...
| Portable exchange | `data-exchange-1` | Verified | Full and thin `.vistrea-pack` exporter/importer over the shared Commit and ObjectRef identity, protocol pack schema and fixtures, deterministic bytes, `LocalDataWorkspace.exchange` composition, and the Host API/CLI/MCP export and binary-import surfaces; readable exports remain later | 7 exchange contracts plus cross-Workspace and dual-Host API pack transfers; commits `b801349` and `e9d1f77` |
...
| iOS Runtime SDK and Demo App | `uikit-runtime-loop-1` | In progress | Canonical models, all shared scenarios, UIKit hierarchy/PNG capture, Debug Inspector, hardened Runtime connection, verified first Snapshot loop, negotiated event streaming, and protected alpha tuning previews exist; automatic UIKit event observation and the SwiftUI adapter remain | 32 Swift tests in Debug and Release (UIKit capture tests also pass on a booted Simulator), 6 Node/Swift interop tests, real iOS E2E; commits `4c67fb2`, `be60c0b`, `7671364`, `74967a6`, `7824f29`, `3511bb5`, and `f5ab283` |
| Android Runtime SDK and Demo App | `android-view-runtime-loop-1` | In progress | Canonical models, all shared scenarios, View/ViewGroup capture, Debug Inspector, protected Runtime connection, Release exclusion, verified first Snapshot loop, negotiated event streaming, and protected alpha tuning previews exist; automatic View event observation and the Compose adapter remain | 6 Node/Kotlin interop tests, 23 connection unit tests, 5 API 36.1 instrumentation tests, release-boundary verification including the tuning controller, and real Android E2E; commits `1925b3a`, `d11dd52`, `4eebcb3`, and `c4295f2` |
| Vistrea Studio | `snapshot-studio-2` | In progress | Native SwiftUI Host status, capture/list, screenshot, 2D tree, node details with Debug alpha tuning previews (apply/revert with verbatim rejection reasons), Review Issues with legal lifecycle transitions, Wiki node creation and revisioned editing, Canvas state details with wiki linking, event timeline, SceneKit 3D Layer Inspector, fixture mode with full write parity, and the production acceptance probe; exploration control from Studio and the comparison workbench remain | 46 Studio tests and Release build; commits `6ccf9f2`, `534517d`, `fa9bf88`, `c62694e`, `85233cb`, `f5ab283`, and `cbefdbe` |
| Agent integrations | `agent-surfaces-3` | In progress | One authenticated Host client exposes 49 operations spanning capture, events, design review, tuning, Screen Graph, exploration Operations, Deep Wiki, validation, build diffs, portable packs, and digest-proved object downloads through the strict JSON CLI, 49 stdio MCP tools, five agent skills (including `vistrea-explore-ui`), and the headless CI gate that validates the newest Snapshot; readable exports remain | Agent adapter round trips, exploration operation integrations, and 4 CI gate integrations; commits `2e1d157`, `e9d1f77`, `914a450`, `7dab236`, and `3b96e82` |
| Automation and exploration | `automation-exploration-1` | Verified | Structural Screen State identity with dedup into one coherent materialized Screen Graph (Engine, Host API, CLI, MCP); semantic action resolution with stale conflicts, risk policy, and confirmation binding; real adb and WebDriverAgent providers behind one port; bounded deterministic exploration with frontier exclusions, frozen graph version tags, and diffs (ADR-0007); both real-input acceptances are verified on dedicated devices, and exploration now runs as an auditable background Operation through the Host (start/poll/cancel) with a configurable adb or WebDriverAgent provider | Graph, automation, WDA wire, exploration, and exploration-operation integration suites plus both passed device acceptances; commits `ec12b1f`, `8417cab`, `80781ba`, `4c96b22`, and `3b96e82` |
...
| Canvas and Deep Wiki | `knowledge-canvas-1` | In progress | The Deep Wiki persists revisioned Markdown nodes with the draft/published/archived lifecycle, resource links, backlinks, related lookups, and text/kind/label/status search through Engine, Host API, CLI, and MCP; Studio renders the Screen State Canvas from the materialized graph, a Deep Wiki search pane, and a SceneKit 3D Layer Inspector; Collection publication (Commit/Ref binding), Wiki editing in Studio, and Canvas curation interactions remain | Knowledge engine integrations with production reopen, agent wiki round trip, and 28 Studio tests; commits `d8dd021` and `85233cb` |
| Validation and build diff | `validation-core-2` | In progress | The `ruleset.vistrea.core` validators judge structural identity coverage, touch targets, labels, geometry sanity, and Screen Graph reachability with persisted runs, exact finding counts, and justified suppressions, now with caller configuration (disabled rules, touch-target threshold) that fails closed on unknown rules and persists into the run for audit; Build Diffs report per-build coverage from observation evidence; baseline-classified regressions remain | Validation and build-diff engine integrations including the configuration cases; commits `0fc4e18`, `cc1c139`, and `d02878c` |
| Vistrea Hub | `hub-pack-relay-1` | In progress | An optional loopback pack relay serves the contract's ref listing/resolution, precondition-guarded `refs:update`, and `packs:import`/`packs:export` over one shared remote Workspace; `HubPackSync` pushes with explicit fast-forwards (never force), fetches, and reports divergent refs as conflicts, validating every remote response and enforcing loopback binds; multi-project namespaces, TLS, RBAC, and collaboration endpoints remain | The Hub sync integration covering push, fetch, byte-identical objects, conflict preservation, and fast-forward; commits `d756cfe` and `7dab236` |

Platform `implementation_status` remains `in-progress` in `examples/scenarios/manifest.json`. Verified per-platform capabilities now cover `runtime.snapshot`, `runtime.connection`, `runtime.events`, `design.tuning`, `automation.actions`, `state-identity.normalization`, and `exploration.graph` on both platforms; `automation.safety` stays `implemented` (the dangerous-action confirmation path has no device evidence yet) and design comparison plus validation scenario coverage remain `planned`.
...
- The dangerous-action confirmation path has integration coverage but no on-device acceptance evidence, so `automation.safety` stays `implemented` in the scenario manifest.
...
- Knowledge Collection publication (Commit/Ref binding), Canvas identity curation (merge/split), exploration control from Studio, the full design comparison workbench (overlay, Studio review mode), and baseline-classified build regressions are not implemented.
- SwiftUI and Compose ship semantic annotation bridges only; the full semantic-tree capture adapters remain future platform work (Compose content currently captures as its single `AndroidComposeView` container).
- CI target-environment orchestration (booting devices inside pipelines) and Hub multi-project namespaces, TLS transport, RBAC, discovery, and collaboration endpoints are not implemented.

## Next milestones

1. Drive exploration from Studio and add Canvas identity curation (merge/split) over the new exploration Operations.
2. Add baseline-classified build regressions and the design comparison workbench (overlay, Studio review mode).
3. Grow the Hub past the single-project loopback relay and build the full SwiftUI/Compose semantic-tree capture adapters.
