...
| Vistrea Studio | `snapshot-studio-1` | In progress | Native SwiftUI Host status, capture/list, screenshot, 2D tree, node details, scenario/build/source context, Runtime event timeline, Review Issues, Screen State Canvas, Deep Wiki search, SceneKit 3D Layer Inspector, fixture mode, and production acceptance probe; editing workflows and exploration control remain | 29 Studio tests and Release build; commits `6ccf9f2`, `534517d`, `fa9bf88`, `c62694e`, `85233cb`, and `f5ab283` |
| Agent integrations | `agent-surfaces-2` | In progress | One authenticated Host client exposes 46 operations spanning capture, events, design review, tuning, Screen Graph, Deep Wiki, validation, build diffs, portable packs, and digest-proved object downloads through the strict JSON CLI, 46 stdio MCP tools, four agent skills, and the headless CI gate that validates the newest Snapshot; exploration sessions and readable exports remain | Agent adapter round trips and 4 CI gate integrations; commits `2e1d157`, `e9d1f77`, `914a450`, and `7dab236` |
| Automation and exploration | `automation-exploration-1` | Verified | Structural Screen State identity with dedup into one coherent materialized Screen Graph (Engine, Host API, CLI, MCP); semantic action resolution with stale conflicts, risk policy, and confirmation binding; real adb and WebDriverAgent providers behind one port; bounded deterministic exploration with frontier exclusions, frozen graph version tags, and diffs (ADR-0007); both real-input acceptances are verified on dedicated devices, including the autonomous exploration segment | Graph, automation, WDA wire, and exploration integration suites plus the passed `pnpm test:e2e:android-real-automation` and `pnpm test:e2e:ios-real-automation`; commits `ec12b1f`, `8417cab`, `80781ba`, and `4c96b22` |
...
| Validation and build diff | `validation-core-1` | In progress | The `ruleset.vistrea.core` validators judge structural identity coverage, touch targets, labels, geometry sanity, and Screen Graph reachability with persisted runs, exact finding counts, and justified suppressions; Build Diffs report per-build Screen State and Transition coverage from observation evidence; rule configuration, pixel comparison, and baseline-classified regressions remain | Validation and build-diff engine integrations plus the agent validation round trip inside 66 Host integrations; commits `0fc4e18` and `cc1c139` |
...
- Knowledge Collection publication (Commit/Ref binding), Wiki editing inside Studio, Canvas curation (merge/split identity, exploration control), validator rule configuration, pixel-level visual comparison, and baseline-classified build regressions are not implemented.
...
- CI target-environment orchestration (booting devices inside pipelines), Host-exposed exploration sessions, and Hub multi-project namespaces, TLS transport, RBAC, discovery, and collaboration endpoints are not implemented.

## Next milestones

1. Expose exploration sessions through the Host so agents and the `vistrea-explore-ui` skill can drive discovery, and grow the Hub past the single-project loopback relay.
2. Build the Studio operation surfaces: tuning preview controls, Review Issue transitions, Wiki editing, and Canvas curation.
3. Deepen validator configuration, baseline-classified build regressions, design comparison depth, and the full SwiftUI/Compose semantic-tree capture adapters.
