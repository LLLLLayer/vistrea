...
2. Expose exploration sessions through the Host so agents and the `vistrea-explore-ui` skill can drive discovery, and grow the Hub past the single-project loopback relay.
3. Deepen Canvas curation, Wiki editing in Studio, validator configuration, baseline-classified build regressions, and the full SwiftUI/Compose semantic-tree capture adapters.
