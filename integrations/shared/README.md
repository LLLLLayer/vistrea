...
This module owns the strict Host Local API client shared by CLI and MCP. It accepts only canonical literal loopback HTTP origins, reads the bearer token from controlled configuration, disables redirects, bounds request and response bodies, applies a request deadline, and strictly decodes Host JSON. Alternate IPv4 spellings such as integer, hexadecimal, octal, or abbreviated forms are rejected before URL normalization.
