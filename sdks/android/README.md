...
The Debug Demo Inspector and protected Host bootstrap exist under `examples/android/VistreaDemoApp/`; its Debug variant records transient banner presentation and dismissal through the bounded `RuntimeEventRecorder`, while the Release variant installs no reporter. `AndroidViewRuntimeTuningController` resolves stable identifiers to live views with the capture adapter's candidate order and previews allowlisted alpha, foreground/background color, font, spacing, content-inset, and corner-radius changes on the main thread. Unsupported view/property combinations reject explicitly, every accepted preview is reversible, and the controller compiles only into the debug and internal variants.
