...
import { createEmptyMemoryState, type MemoryState } from "./state.js";
import {
...
export class MemoryDataStore implements WorkspaceDataSource {
  ...
  beginUnitOfWork(mode: UnitOfWorkMode): DataUnitOfWork {
    ...
    const unit = new MemoryDataUnitOfWork(
      this,
      id,
      mode,
      this.#generation,
  ...
  _commit(unit: MemoryDataUnitOfWork): void {
  ...
  _rollback(unit: MemoryDataUnitOfWork): void {
    this.#openUnits.delete(unit.id);
  }
...
class MemoryDataUnitOfWork implements DataUnitOfWork, UnitContext {
  ...
  constructor(
    ...
    this.snapshotVersion = `memory:${baseGeneration}`;
    this.snapshots = new MemorySnapshotRepository(this);
    this.observations = new MemoryObservationRepository(this);
    this.runtimeEvents = new MemoryRuntimeEventRepository(this);
    this.screenGraph = new MemoryScreenGraphRepository(this);
    this.wiki = new MemoryWikiRepository(this);
    this.designReviews = new MemoryDesignReviewRepository(this);
    this.validation = new MemoryValidationRepository(this);
    this.operations = new MemoryOperationRepository(this);
    this.versions = new MemoryVersionRepository(this);
  }
...
abstract class BoundMemoryRepository {
...
class MemorySnapshotRepository extends BoundMemoryRepository implements SnapshotRepository {
...
class MemoryObservationRepository extends BoundMemoryRepository implements ObservationRepository {
...
class MemoryRuntimeEventRepository extends BoundMemoryRepository implements RuntimeEventRepository {
...
class MemoryScreenGraphRepository extends BoundMemoryRepository implements ScreenGraphRepository {
...
class MemoryWikiRepository extends BoundMemoryRepository implements WikiRepository {
...
class MemoryDesignReviewRepository
  extends BoundMemoryRepository
...
class MemoryValidationRepository extends BoundMemoryRepository implements ValidationRepository {
...
class MemoryOperationRepository extends BoundMemoryRepository implements OperationRepository {
...
class MemoryVersionRepository extends BoundMemoryRepository implements VersionRepository {
  ...
  *reachableObjects(commitIds: readonly string[]): Iterable<ObjectRef> {
    ...
    while (pending.length > 0) {
      ...
      for (const hash of commit.manifest.object_hashes) {
        ...
        const object = (this.unit as MemoryDataUnitOfWork).store._verifiedObject(hash);
        if (object === undefined) {
